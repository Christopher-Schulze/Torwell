<script lang="ts">
  ...
  function saveMaxMemory() {
    const limit = parseInt(String(maxMemoryMB));
    if (!isNaN(limit) && limit > 0) {
      uiStore.actions.setMaxMemory(limit);
    }
  }

  function saveMaxCircuits() {
    const limit = parseInt(String(maxCircuits));
    if (!isNaN(limit) && limit > 0) {
      uiStore.actions.setMaxCircuits(limit);
    }
  }
