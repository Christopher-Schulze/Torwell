...
<div class="bg-black/20 rounded-xl p-6">
	<div class="flex items-center justify-between gap-6">
		...
		<!-- Traffic and Ping Section -->
		<div class="flex items-center gap-4">
			<!-- Traffic Display -->
			<div class="bg-black/50 rounded-lg px-2 py-1 h-8 flex items-center gap-2 min-w-[100px]">
				<Activity class="w-4 h-4 text-green-400" />
				<span class="text-xs text-white font-medium">{formatTraffic(totalTrafficMB)}</span>
			</div>
			
			<!-- Ping Display -->
			<div class="bg-black/50 rounded-lg px-2 py-1 h-8 flex items-center gap-2 min-w-[80px]">
			...
			<button 
				class="w-8 h-8 bg-black/50 rounded-lg hover:bg-black/60 transition-all flex items-center justify-center {isPinging ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}"
			...
			>
				{#if isPinging}
					...
					<div class="relative w-full h-full flex items-center justify-center">
						<div class="absolute w-2 h-2 bg-blue-400/60 rounded-full animate-ping"></div>
						<div class="absolute w-3 h-3 bg-blue-400/40 rounded-full animate-ping" style="animation-delay: 0.2s;"></div>
						<div class="absolute w-4 h-4 bg-blue-400/20 rounded-full animate-ping" style="animation-delay: 0.4s;"></div>
						<div class="w-1.5 h-1.5 bg-blue-400 rounded-full"></div>
					</div>
				{:else}
					<!-- Static concentric circles -->
					<div class="relative w-full h-full flex items-center justify-center">
						<div class="absolute w-4 h-4 border border-white/20 rounded-full"></div>
						<div class="absolute w-3 h-3 border border-white/30 rounded-full"></div>
						<div class="absolute w-2 h-2 border border-white/40 rounded-full"></div>
