...
impl<C: TorClientBehavior> AppState<C> {
    ...
    /// Update the maximum allowed memory usage in MB
    pub async fn set_max_memory_mb(&self, limit: u64) {
        *self.max_memory_mb.lock().await = limit;
    }

    /// Update the maximum allowed number of circuits
    ...
    pub async fn update_metrics(
        ...
        *self.network_throughput.lock().await = network;
        ...
        let max_mem = *self.max_memory_mb.lock().await;
        let max_circ = *self.max_circuits.lock().await;
        ...
        if memory_mb <= max_mem && circuits <= max_circ {
